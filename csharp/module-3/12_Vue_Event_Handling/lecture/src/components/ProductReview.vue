<template>
  <div class="main">
    ...
    <button v-on:click="showForm = true" v-show="showForm ===false">Show Form</button>
...
export default {
  ...
  computed: {
    ...
    //filteredReviews computer property will return an [] of reviews based on the value of the filter variable. 
    filteredReviews() {
      let filtered = this.reviews.filter((review) => {
        //if the filter variable is 0, l the reviews should be returned. 
        //if variable  != 0, only return reviews with the filter value
  ...
  methods: { //behaviors go here
    //don't need function keyword but that's what this is
    addNewReview() {
      this.reviews.unshift(this.newReview);
      //add the newReview object (with 2-way binding to template) to the reviews []
      this.resetForm();
    },
    resetForm(event) {
      this.newReview = {};
