<template>
  <div class="main">
       <h1> {{ title }}</h1>
       <p> {{ description }} </p>
       <p> A delightful book by {{kimBarry}}</p>
...
<script>
export default { 
    name: 'product-review',   //component always has a name first
    data() {  //data function returns some kind of object
        return {
    ...
    computed: {
        averageRating() {
            //add the ratings and divide by the number of ratings
